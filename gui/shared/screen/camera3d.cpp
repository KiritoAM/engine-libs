...
namespace gui
{
	...
	void CAMERA3D::move( const core::FVECTOR3D& movement )
	{
		...
		m_transform.set_position( m_local_position );
		on_transform_changed();
	}

	void CAMERA3D::rotate( const core::FVECTOR2D& delta )
	{
		const core::FVECTOR2D mouse_delta = delta * MOUSE_SENSITIVITY;
		...
		m_mouse_rotation.y = std::clamp( m_mouse_rotation.y, -90.0f, 90.0f );
