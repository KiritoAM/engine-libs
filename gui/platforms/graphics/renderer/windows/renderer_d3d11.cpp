...
namespace gui
{
	RENDERER_D3D11::RENDERER_D3D11()
		...
		, m_rasterizer_cull_back_solid{ m_device, RHI_Cull_Front, RHI_Fill_Solid, true, false, false }
	{
		create_shaders();
	...
	void RENDERER_D3D11::pass_gbuffer( const WINDOW_ID_TYPE window_id )
	{
		...
		for ( auto& [map_key, viewport] : window_context.viewports )
		{
			...
			if ( m_device.get_context()->begin_render_pass( pipeline_state ) )
			{
				...
				{
					const auto transform_matrix{ world_object->get_transform().get_matrix() };
					...
					const auto num_vertices = world_object->get_mesh()->get_vertices().size();

					s_dxgi_info_manager.Set();
					m_device.get_context()->draw_indexed( static_cast<uint32_t>(num_vertices), 0u, 0u );
